...
const ISearch = () => (
  <Ic d={["M21 21l-4.35-4.35", "M11 19a8 8 0 1 0 0-16 8 8 0 0 0 0 16z"]} />
);
...
// ─── Stat Card ───────────────────────────────────────────────────────────────
function StatCard({ label, value, sub, icon, tag }) {
  return (
    <div className="s-card rounded-2xl p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        {tag && (
          <span className="text-[10px] px-2 py-0.5 rounded-full font-bold s-tag">
            {tag}
          </span>
        )}
      </div>
      <div>
        <p className="text-[32px] font-black leading-none mb-1 s-val">
          {value}
        </p>
...
function CRow({ c, onView }) {
  ...
  return (
    <div
      className="c-row flex items-center gap-3 px-4 py-3.5 cursor-pointer"
      onClick={() => onView(c)}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-[11px] flex items-center gap-1 c-meta">
            <IClock s={11} /> {relDate(c.updatedAt)}
          </span>
          <span className="hidden sm:inline-flex text-[11px] items-center gap-1 c-meta">
            <IBuilding s={11} /> {c.dept}
          </span>
        </div>
      </div>
      <div className="flex items-center gap-2 flex-shrink-0">
        <span
          className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-[11px] font-bold"
        ...
        <div className="w-7 h-7 rounded-lg flex items-center justify-center view-btn">
          <IChevR />
        </div>
      </div>
    </div>
...
function DetailDrawer({ complaint, onClose, onNavigate }) {
  ...
  const scoreColor =
    complaint.evidenceScore >= 70
      ? "#1A7F5A"
      : complaint.evidenceScore >= 40
        ? "#2B6CB0"
        : "#8892A4";
  ...
  const UPDATES = Array.from({ length: complaint.updates || 0 }, (_, i) => ({
    date: new Date(
      new Date(complaint.updatedAt).getTime() -
        (complaint.updates - 1 - i) * 86400000 * 2,
    ...
    msg:
      ...
      ][i] || "Status updated by department",
    by:
      ["System", "Control Room", "Dept. Head", "Field Officer", "Dept. Head"][
        i
      ] || "Department",
  }));
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      {/* Drawer panel */}
      <div
        className="drawer relative w-full max-w-md h-full flex flex-col overflow-hidden"
      ...
      >
        ...
        <div
          ...
          style={{ background: "white", borderBottom: "1.5px solid #F0F3FA" }}
        >
          <div
          ...
          <span
            className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-[11px] font-bold flex-shrink-0"
            style={{ background: st.bg, color: st.color }}
          >
            <span
              className="w-1.5 h-1.5 rounded-full"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          ...
          <div className="flex gap-3">
            <div
              className="flex-1 p-3.5 rounded-xl flex flex-col gap-1"
              style={{ background: st.bg }}
            >
              <p
                className="text-[9px] font-black uppercase tracking-wider"
                style={{ color: st.color + "99" }}
              >
                Status
              </p>
            ...
            >
              <p
                className="text-[9px] font-black uppercase tracking-wider"
                style={{ color: sv.color + "99" }}
              >
                Severity
              </p>
              <span className="text-sm font-black" style={{ color: sv.color }}>
                {complaint.severity}
              </span>
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{ background: "#F0F3FA", color: "#000000" }}
                >
                  <ISparkle s={12} />
                </div>
                <span
            ...
            >
              <div
                ...
                style={{
                  width: `${complaint.evidenceScore}%`,
                  background: "#000000",
                }}
              />
            </div>
            <div className="flex flex-wrap gap-1.5 mt-3">
              {complaint.evidenceScore >= 50 && (
                <span className="evi-chip">📍 Location</span>
              )}
              {complaint.evidenceScore >= 60 && (
                <span className="evi-chip">📷 Photos</span>
              )}
              {complaint.evidenceScore >= 80 && (
                <span className="evi-chip">📝 Description</span>
              )}
              {complaint.evidenceScore < 50 && (
                <span className="evi-chip-warn">⚠️ Add evidence</span>
              )}
            </div>
          </div>
          ...
          >
            ...
            {[
              ...
              },
              {
                icon: <IClock s={13} />,
                label: "Updated",
                val: fmtDate(complaint.updatedAt),
              },
            ].map(({ icon, label, val }) => (
          ...
          >
            ...
            <div className="relative">
              ...
              <div
                ...
                style={{ width: `${progressPct}%`, background: "#000000" }}
              />
                ...
                {STEPS.map((s, i) => {
                  ...
                  return (
                    <div key={s} className="flex flex-col items-center gap-2">
                      <div
                        className={`w-7 h-7 rounded-full flex items-center justify-center z-10 transition-all duration-300 ${done ? "step-done" : "step-pend"} ${active ? "step-active-ring" : ""}`}
                      >
                        {done ? (
          ...
          {UPDATES.length > 0 && (
            ...
            >
              <p
                ...
                style={{ color: "#8892A4" }}
              >
                Activity · {UPDATES.length} update
                {UPDATES.length !== 1 ? "s" : ""}
              </p>
              <div className="relative">
                <div
                ...
                <div className="space-y-4">
                  {UPDATES.map((u, i) => (
                    <div key={i} className="relative flex gap-4 pl-10">
                      <div
                        className={`absolute left-0 w-7 h-7 rounded-full flex items-center justify-center z-10 ${i === UPDATES.length - 1 ? "step-done" : "step-pend"}`}
                      >
                        {i === UPDATES.length - 1 ? (
                          <ICheck s={12} stroke="white" />
                        ) : (
          ...
          {complaint.status === "Pending" && (
            ...
            >
              ...
              <div>
                <p className="text-xs font-black" style={{ color: "#000000" }}>
        ...
        {/* Sticky footer — link to full details page */}
        <div
          className="flex-shrink-0 px-5 py-4"
          style={{ background: "white", borderTop: "1.5px solid #F0F3FA" }}
        >
          <button
            onClick={() => {
              onClose();
              onNavigate(`/complaint/${complaint.id}`);
            }}
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm transition-all cursor-pointer border-none"
...
export default function CitizenDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const active = MOCK_COMPLAINTS.filter((c) =>
      ["Assigned", "In Progress"].includes(c.status),
    ).length;
    const avgScore = Math.round(
      MOCK_COMPLAINTS.reduce((s, c) => s + c.evidenceScore, 0) / total,
    );
    const resolveRate = Math.round((resolved / total) * 100);
    return { total, resolved, active, avgScore, resolveRate };
  }, []);
  ...
  const donutData = [
    ...
    {
      label: "Pending",
      value: MOCK_COMPLAINTS.filter((c) => c.status === "Pending").length,
      color: "#D1D5DB",
    },
  ];
    ...
    list.sort(
      {
        ...
        severity: (a, b) =>
          ["Critical", "Severe", "Moderate", "Minor"].indexOf(a.severity) -
          ["Critical", "Severe", "Moderate", "Minor"].indexOf(b.severity),
        score: (a, b) => b.evidenceScore - a.evidenceScore,
      }[sortBy],
    );
    return list;
      ...
      >
        <div className="max-w-6xl mx-auto px-4 py-3.5 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold cursor-pointer border-none"
            style={{ background: "#000000", color: "white" }}
          >
            <IPlus /> <span className="hidden sm:inline">New Complaint</span>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="fu">
          ...
          >
            ...
            <span style={{ color: "#B0B8C9" }}>
              Here's your civic activity.
            </span>
          </h1>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 fu1">
          <StatCard
            label="Total Filed"
            value={stats.total}
            icon={<IFile />}
            sub={`Since ${MOCK_CITIZEN.joinedDate}`}
          />
          <StatCard
            label="Resolved"
            value={stats.resolved}
            icon={<ICheck />}
            tag={`${stats.resolveRate}%`}
          />
          <StatCard
            label="Active"
            value={stats.active}
        ...
        {/* Charts */}
        <div className="grid md:grid-cols-3 gap-4 fu2">
          {/* Profile */}
          ...
          <div className="chart-card flex flex-col">
            ...
            <div className="flex-1 flex items-center justify-around gap-4">
              <div className="relative flex-shrink-0">
                <Donut data={donutData} size={108} thick={13} />
              ...
              <div className="space-y-2.5">
                {donutData.map((s) => (
                  <div key={s.label} className="flex items-center gap-2">
                    ...
                    <span
                      className="text-xs font-black ml-auto pl-4"
                      style={{ color: "#000000" }}
                    >
                      {s.value}
                    </span>
          ...
          <div className="flex flex-col gap-3">
            <div className="chart-card flex-1">
              ...
              <Bar data={monthlyData} height={76} />
            </div>
            <div className="chart-card flex-1">
              <p
                className="text-[10px] font-black uppercase tracking-wider mb-3"
              ...
              <Bar
                data={(() => {
                  ...
                  MOCK_COMPLAINTS.forEach((c) => {
                    cnt[c.category] = (cnt[c.category] || 0) + 1;
                  });
                  return Object.entries(cnt).map(([cat, val]) => ({
                    label: cat.split(" ")[0],
                    value: val,
                    color: "#000000",
                    dim: val < 2,
        ...
        <div className="panel fu3">
          <div
            className="px-4 py-4"
            style={{ borderBottom: "1.5px solid #F0F3FA" }}
          >
            <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
              ...
              <button
                onClick={() => navigate("/file-complaint")}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold cursor-pointer border-none"
                style={{ background: "#F0F3FA", color: "#000000" }}
              >
                <IPlus /> File New
              </button>
            ...
            {activeTab === "complaints" && (
              <div className="flex flex-col sm:flex-row flex-wrap gap-2">
                <div
                  className="flex items-center gap-2 flex-1 min-w-0 px-3 py-2 rounded-xl"
                  style={{
                    background: "#F7F9FC",
          ...
          {activeTab === "analytics" && (
            <div className="p-5 grid md:grid-cols-2 gap-5">
              <div>
                ...
                <div className="space-y-3">
                  {["Critical", "Severe", "Moderate", "Minor"].map((sev) => {
                    const count = MOCK_COMPLAINTS.filter(
                      (c) => c.severity === sev,
                    ).length;
                      ...
                      <div key={sev}>
                        <div className="flex justify-between mb-1.5">
                          ...
                          <span
                            ...
                            style={{
                              color:
                                cfg.color !== "#6B7280" ? cfg.color : "#000000",
                            }}
                          >
                            {count} ({pct}%)
                          </span>
                      ...
                      .map(([dept, count]) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span
                              className="text-xs font-black w-4 text-right"
                              style={{ color: "#000000" }}
                            >
                              {count}
                            </span>
              ...
              <div
                className="md:col-span-2 p-4 rounded-2xl flex items-center gap-4"
                style={{ background: "#F0F3FA", border: "1.5px solid #E4E8F0" }}
              >
                <div
                ...
                <div>
                  ...
                  >
                    ...
                    with high evidence scores
                  </p>
                  <p className="text-xs mt-0.5" style={{ color: "#8892A4" }}>
                    Attach GPS location and photos when filing to boost your
                    score and speed up resolution.
                  </p>
                </div>
              </div>
            </div>
